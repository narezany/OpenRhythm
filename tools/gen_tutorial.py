#!/usr/bin/env python3
"""Open Rhythm tutorial: a short synthesised track plus its teaching chart.
The first ~35 seconds are text hints and single "catch the cube" notes, then a
simple melody. Run from the project root: python3 tools/gen_tutorial.py
"""
import json
import math
import os
...
def synth(freq, dur, vol=0.4, attack=0.01, release=0.08):
    """Soft synth pluck: a sine plus its fifth, exponential decay."""
    n = int(dur * SR)
    buf = []
    for i in range(n):
        t = i / SR
...
BPM = 100.0
BEAT = 60.0 / BPM
INTRO_END = 32.0   # end of the teaching section

events = []
# --- chord bed: sets the mood without competing with the lead ---
...
while t < 64.0:
    ...
    for m in ch:
        events.append((t, m - 12, BEAT * 3.6, 0.10))   # bass
        events.append((t, m, BEAT * 3.2, 0.07))        # pad
    events.append((t + 0.0, ch[0] + 24, 0.18, 0.05))   # tick
    events.append((t + 2 * BEAT, ch[1] + 24, 0.18, 0.05))
    t += 4 * BEAT
    bar += 1

# --- part 1: "catch the cube", four slow single notes ---
catch_times = [12.0, 18.0, 23.0, 28.0]
catch_notes = [69, 72, 74, 76]   # A4 C5 D5 E5, pleasant and easy to hear
for tt, mm in zip(catch_times, catch_notes):
    events.append((tt, mm, 0.5, 0.34))
    events.append((tt + 0.12, mm + 12, 0.25, 0.10))   # sparkle

# --- part 2: a simple melody, 16 beats ---
...
for i, m in enumerate(melody):
    ...
    t += BEAT
# the melody again an octave up and denser
...
# --- chart: tutorial hints plus a simple layout ---
notes = []
# "catch the cube" phase: a note exactly where the sound is
CELLS = [4, 2, 6, 8]   # centre, top-right, bottom-left, bottom-centre
for i, tt in enumerate(catch_times):
    notes.append({"t": round(tt, 3), "cell": CELLS[i], "s": 1.35})  # a big cube

# melody phase: one note per sound, cycling through cells
